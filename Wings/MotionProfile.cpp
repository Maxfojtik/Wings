...
MotionProfile::MotionProfile(float aVelocityMax, float aAccelerationMax, short aMethod, int aSampleTime, float dampFactor)
: maxVelocity(aVelocityMax), maxAcceleration(aAccelerationMax), method(aMethod), sampleTime(aSampleTime), dampFactor(dampFactor)
{
	init();
}
...
void MotionProfile::calculateTrapezoidalProfile(float setpoint) {
	...
	if (((velocity * velocity) / maxAcceleration) / (2*dampFactor) >= abs(setpoint - position)) {
		if (velocity < 0) {
